...
@section('content_header')
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1 class="m-0">Daily Attendance Checkup</h1>
            </div>
            <div class="col-sm-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('patient_id') is-invalid @enderror" id="patient_id" name="patient_id" required>
                                            ...
                                            @foreach($patients as $pt)
                                                <option value="{{ $pt->id }}" {{ old('patient_id') == $pt->id ? 'selected' : '' }}>{{ $pt->name }} - {{ $pt->ward ?? 'No Ward' }}</option>
                                            @endforeach
                                        </select>
                                        @error('patient_id')
                                            <span class="invalid-feedback">{{ $message }}</span>
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="ward" name="ward" value="{{ old('ward') }}" placeholder="e.g., Ward A">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="days_under_care">Days Under Care</label>
                                        <input type="number" class="form-control" id="days_under_care" name="days_under_care" value="{{ old('days_under_care', 0) }}" min="0">
                                        <span class="text-muted"><small>Auto-calculated</small></span>
                                    </div>
                                </div>
                            </div>

                            <hr>
                            <h5>Observations & Reports</h5>
